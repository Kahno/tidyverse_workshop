...
Tidyverse (https://www.tidyverse.org/) is a collection of packages for R language (https://www.r-project.org/). It is composed of core tidyverse packages:
...
There are many more packages in tidyverse such as lubridate, stringr, forcats, etc... In this workshop we will do a short overview of five core packages. The core packages include:

## tibble

An alternative for data.frame called **tibble** or **tbl_df**. Tibble has mainly improve prints and always returns a tibble if we subset it.

## readr

Package readr is a package for reading data from .csv files with improved defaults over `read.csv()` function from **utils** package. 

## tidyr

Package for tidiyng data. This means that we want to have a data set where each table has one kind of observations. Each column in each table represents an attribute and each row an observation. Also includes functions to transform data from long to wide format and vice versa.

## dplyr

Main package for transforming data to the desired structured. Tidiying and transforming data is also called wrangling and is done at the same time.

## purrr

An alternative to Rs in-built `apply()` functions. Allows to run same function over list or vectors.

## ggplot2

A package used make beautiful graphs.
...
We will focus on reading .csv files and compare the process to the in-build functions. Package **readr** also allows to read excel spreadsheets, SPSS data, xml files, etc...

## readr vs utils

Both packages are used to read data from a .csv file. Lets load census.csv dataset with both packages. Tidyverse tries to remove the use of '.' from variable and function names, so you can recognize tidyverse functions because they use underscore ('_') instead of dot ('.') in names.
...
Both functions successfully loaded the file with default parameters but there are few differences. Lets check the class of both variables.
...
Tibble also shows some extra information as the size of the table and class of columns. Looking at the class of `census.data` we can see that the classes of attribtes differ. We would need to use parameter `stringsAsFactors = FALSE` while using `read.csv()` to avoid conversion to factors at read time.

```{r, eval=TRUE, echo=TRUE}
lapply(data_list, function(x) {sapply(x, class)})
```

Note that the utils `read.csv()` function also changed the name of 'education-num' attribute to 'education.num'. This means that you can't just replace `read.csv()` with `read_csv()` because it will lead to errors later in the script.

OK, these are the differenes while reading the file. Lets write the data back...
...
You can see that the basic R has quite nonintuitive defaults since by writing and loading a table we constructed a new column 'X' with unique id. The basic R file is also bigger, around 4.5MB, while tidyverse version is only 3.7MB. 
...
Table read using `read.csv()` still has "?" value. The reason is that `read.csv()` didn't remove leading spaces. We should use `na.strings = " ?"`.
...
Note that the types and names of attributes are now different as we would got them while reading directly from file since `read.csv()` function changed their names. In html markdown it is also visible that the `read.csv()` added an extra space to all factors.
...
First lets just select a small subset of atttributes from movies dataset. This can be done with `select()'.
...
Or we can see all the movies Tom Hanks played one of the leading roles.

```{r, eval=TRUE}
filter(movies_small, Actor_name == "Tom Hanks")
```

The above step is called transforming a table from wide format to long format.
...
If you need permutation of all the values of few attributes you can use `expand()`.
...
We lost information for around 1300 movies this way. `drop_na()` removes any observation that has a missing values anywhere in the table. We can only remove observations that are missing specific value. 

```{r, eval=TRUE}
drop_na(movies, country)
```

We only removed 5 movies which have a missing country field.
...
```{r, eval=TRUE}
replace_na(movies, list(duration = round(mean(movies$duration, na.rm = TRUE)), num_critic_for_reviews = 0))
```
...
We will now go to the last part which is transforming data. We will again use census data.
...
We can also use this to change the type of columns. Lets change every character column to factor.
